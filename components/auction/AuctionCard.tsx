...
export default function AuctionCard({auction, isWatched = true}: { auction: Auction, isWatched: boolean }) {
    const [showRegistrationModal, setShowRegistrationModal] = useState(false);
    const [lotCount,] = useAtom(lotsCountByAuctionAtom)
    ...
    const getRegistrationComponent = () => {
        ...
        if (!auction.is_auction_registered) {
            return (
                <Button size={'xs'} onClick={() => setShowRegistrationModal(true)} mr={10} color={'primary.9'}
                        radius={'sm'} h={50}>
                    REGISTER TO BID
                </Button>
            );
                ...
                <Grid.Col span={{xs: 12, sm: 4}}>
                    <Stack>
                        ...
                        <Flex justify={'flex-end'}>
                            ...
                            <Center>
                                <Link href={`/UpcomingAuctions/${auction.id}/Lots`}
                                      passHref>
                                    <Button size={'xs'} color={'gray.7'} radius={'sm'}
                                            h={50}>Click To View {auction.lots} Lots</Button>
                                </Link>
                            </Center>
                        </Flex>
                    </Stack>
